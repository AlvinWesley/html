...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['submitUserEdits'])) {
        if ($_POST['submitUserEdits'] === 'submit') {
            if (isset($_POST['userId']) && isset($_POST['userStatus'])) {
                ...
                $stmt = $conn->prepare("UPDATE USER_DETAILS_TBL SET regStatus = ? WHERE user_id = ?");
        ...
        } elseif ($_POST['submitUserEdits'] === 'click') {
            if (isset($_POST['deptID']) && isset($_POST['userId'])) {
                $deptID = $_POST['deptID'];
                $userId = $_POST['userId'];

                // Update user department
                $stmt = $conn->prepare("UPDATE USER_LOGIN_TBL SET department_id = ? WHERE user_id = ?");
                $stmt->bind_param("ii", $deptID, $userId);
                if ($stmt->execute()) {
                    echo "User department updated successfully";
                } else {
                    echo "Error updating user department: " . mysqli_error($conn);
                }
            }
        } elseif ($_POST['submitUserEdits'] === 'delete') {
            if (isset($_POST['userId'])) {
                $userId = $_POST['userId'];

                // Delete user from USER_LOGIN_TBL
                $stmt = $conn->prepare("DELETE FROM USER_LOGIN_TBL WHERE user_id = ?");
        ...
        } elseif ($_POST['submitUserEdits'] === 'deactivate') {
            if (isset($_POST['userId']) && isset($_POST['userStatus'])) {
                $userId = $_POST['userId'];
                $userStatus = $_POST['userStatus'];

                // Deactivate user by updating regStatus
                $stmt = $conn->prepare("UPDATE USER_DETAILS_TBL SET regStatus = ? WHERE user_id = ?");
                $stmt->bind_param("si", $userStatus, $userId);
                if ($stmt->execute()) {
                    echo "User deactivated successfully";
    ...
    // Fetch department and user details
    $stmtDept = $conn->prepare("SELECT department_id, department_name FROM DEPARTMENT_TBL");
    $stmtDept->execute();
    $departments = $stmtDept->get_result()->fetch_all(MYSQLI_ASSOC);
    ...
    INNER JOIN USER_LOGIN_TBL ON USER_DETAILS_TBL.user_id = USER_LOGIN_TBL.user_id
    INNER JOIN DEPARTMENT_TBL ON USER_LOGIN_TBL.department_id = DEPARTMENT_TBL.department_id
    ORDER BY USER_DETAILS_TBL.user_id ASC");
    $stmtUsers->execute();
    $users = $stmtUsers->get_result()->fetch_all(MYSQLI_ASSOC);
    // Prepare response for user and department details
    $myResults = array("results" => $users, "departments" => $departments);
    echo json_encode($myResults);
}
